...
namespace image_processing
{
    ...
    private:
        ...
        sub = private_nh.subscribe("compressed_image", 1, &DecoderNodelet::imageCallback, this);
        }


        void imageCallback(const sensor_msgs::CompressedImageConstPtr& msg)
        {
        cv::Mat image = cv::imdecode(cv::Mat(msg->data),CV_LOAD_IMAGE_UNCHANGED);//convert compressed image data to cv::Mat
